                            ...
                            <tbody>
                                @forelse ($coaches as $index => $coach)
                                    <tr>
                                        ...
                                        @endforeach
                                        <td class="text-center ">
                                            {!! array_sum(array_map(fn($m) => ($m['kehadiran_di_unit'] ?? 0), $coach['months']))==0?"-":"<strong class='text-success'>".array_sum(array_map(fn($m) => ($m['kehadiran_di_unit'] ?? 0), $coach['months']))."</strong> / ".array_sum(array_map(fn($m) => ($m['total_sesi_unit_bulan'] ?? 0), $coach['months'])) !!}<br>
                                            <span class="badge badge-success">{{ array_sum(array_map(fn($m) => ($m['kehadiran_di_unit'] ?? 0), $coach['months'])) > 0 ? number_format((array_sum(array_map(fn($m) => ($m['kehadiran_di_unit'] ?? 0), $coach['months'])) / array_sum(array_map(fn($m) => ($m['total_sesi_unit_bulan'] ?? 0), $coach['months']))) * 100, 0) . '%' : '' }}</span>
                                        </td>
                                        <td class="text-center ">
                                            {!! array_sum(array_map(fn($m) => ($m['kehadiran_di_kalideres'] ?? 0), $coach['months']))==0?"-":"<strong class='text-success'>".array_sum(array_map(fn($m) => ($m['kehadiran_di_kalideres'] ?? 0), $coach['months']))."</strong> / ".array_sum(array_map(fn($m) => ($m['total_sesi_kalideres_bulan'] ?? 0), $coach['months']))!!}<br>
                                            <span class="badge badge-success">{{ array_sum(array_map(fn($m) => ($m['kehadiran_di_kalideres'] ?? 0), $coach['months'])) > 0 ? number_format((array_sum(array_map(fn($m) => ($m['kehadiran_di_kalideres'] ?? 0), $coach['months'])) / array_sum(array_map(fn($m) => ($m['total_sesi_kalideres_bulan'] ?? 0), $coach['months']))) * 100, 0) . '%' : '' }}</span>
                                        </td>
